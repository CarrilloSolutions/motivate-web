...
// Convert input like: "#drive, power   focus" -> ["drive","power","focus"]
...
export default function AdminPage() {
  ...
  // Build per-file form rows when selection changes
  useEffect(() => {
    ...
    setPerFile(
      Array.from(files).map((f) => ({
        name: f.name,
        title: f.name.replace(/\.[^/.]+$/, ""), // default to base filename
        hashtagsText: "",
      }))
    );
  }, [files]);
  ...
  const uploadAll = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      // per-file form data
      const meta = perFile[i] ?? { title: safeName.replace(/\.[^/.]+$/, ""), hashtagsText: "" };
      ...
      try {
        await new Promise<void>((resolve, reject) => {
          ...
          task.on(
            ...
            async () => {
              try {
                // make sure metadata is set too (belt & suspenders)
                ...
                try {
                  await addDoc(collection(db, "videos"), {
                    ...
                    hashtags, // <— array of strings, validated in rules
                  ...
                  if (/app.?check/i.test(msg)) {
                    hint =
                      " — Firestore App Check is enforced and the client is missing/invalid token. Ensure App Check is initialized in lib/firebase.ts and re-deploy.";
                  } else if (/insufficient permissions/i.test(msg)) {
                    hint =
                      " — Check Firestore rules for /videos writes and that your account is listed as admin.";
                  }
                  setStatus(
        ...
        <div className="pt-6 pb-28">
          <div className="mx-auto max-w-3xl px-4 space-y-4">
                  ...
                  >
                    ...
                    <div className="md:col-span-2">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateMeta(idx, { hashtagsText: e.target.value })
                        }
                        placeholder="#drive, #power, #focus"
                        className="w-full rounded-md bg-white/5 border border-white/10 px-3 py-2"
                      />
                      <p className="text-[10px] opacity-60 mt-1">
                        Use commas or spaces. “#” optional.
                      </p>
                    </div>
                  </div>
                ))}
            ...
            <p className="text-xs opacity-60">
              Title + hashtags are stored in each Firestore video document.
            </p>
          </div>
        </div>
        <Navbar />
